 ...
 *
 * `printer_enabled` ('1'/'0') dice si el negocio usa impresora; apagarla NO borra la
 * conexión, así se puede volver a activar tal como estaba. Sin impresora, la caja no
 * muestra avisos de "ticket no impreso".
 *
 * `printer_interface`, en el formato que entiende el servidor (`src/main/services/printer.ts`):
...
type Mode = 'network' | 'windows' | 'custom' | null
...
const MODES: { value: Exclude<Mode, null>; label: string; help: string }[] = [
  {
    value: 'windows',
    label: 'USB en la PC servidor',
    help: 'Enchufada a la PC servidor, con su driver instalado en Windows.'
  },
  {
    value: 'network',
    label: 'De red (cable o WiFi)',
    help: 'Tiene su propia IP en la red del negocio.'
  },
  { value: 'custom', label: 'Avanzado', help: 'Conexión escrita a mano (soporte técnico).' }
]
...
export function ImpresoraSection({
  ...
  useEffect(() => {
    if (!enabled || parse(value).mode !== 'windows') return
    listImpresoras()
      .then(setPrinters)
      .catch(() =>
        setPrinters({ supported: true, printers: [], error: 'No se pudo leer la lista.' })
  ...
  return (
    <section className="rounded-xl border border-border p-4">
      ...
      <div className="mt-3">
        ...
        <div role="radiogroup" className="inline-flex rounded-lg border border-border p-0.5">
          {[
            { v: false, label: 'No' },
            { v: true, label: 'Sí' }
          ].map((o) => (
            <button
              ...
              type="button"
              role="radio"
              aria-checked={enabled === o.v}
              onClick={() => {
                setTest(null)
                onEnabledChange(o.v)
                if (o.v && mode === 'windows' && !printers) void loadPrinters()
              }}
              className={cn(
                'rounded-md px-5 py-1.5 text-sm font-medium transition',
                enabled === o.v ? 'bg-primary text-primary-foreground' : 'hover:bg-secondary'
              )}
            >
              {o.label}
            </button>
      ...
      {enabled && (
        <>
          ...
          {mode === 'windows' && (
            <div className="mt-4">
              <span className="mb-1 block text-sm font-medium">
                Impresora instalada en el servidor
              </span>
